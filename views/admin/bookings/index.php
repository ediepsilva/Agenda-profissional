...
$link = static fn (array $over) => url('/admin/reservas', array_filter($over + ['status' => $status ?: null, 'q' => $q ?: null, 'periodo' => $period], static fn ($v) => $v !== null && $v !== ''));
?>
...
<form method="get" action="<?= e(url('/admin/reservas')) ?>" class="filters panel">
    <div>
        ...
        <input type="search" id="f-q" name="q" value="<?= e($q) ?>" placeholder="Nome ou WhatsApp">
    </div>
    <div>
...
<table class="table">
    ...
    <?php foreach ($rows as $b): ?>
        <tr>
            ...
            <td data-label="Serviço"><?= e($b['service_name']) ?></td>
            <td data-label="Profissional"><span class="dot" style="background: <?= e($b['professional_color']) ?>"></span> <?= e($b['professional_name']) ?></td>
